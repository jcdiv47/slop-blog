...
To write a new blog post, create a markdown (or MDX) file inside the `src/content/posts/` directory. Here I am just testing[^1] the new footnote.

[^1]: The footnote body. Supports **inline markdown**, links, etc.
...
<details>
<summary>Custom heading</summary>  
Content to collapse...  
</details>
...
Frontmatter is the main place to store metadata about a blog post.
:aside[This is testing paragraph level floating note.]
It lives at the top of the file in YAML format. Read more about frontmatter and its usage in [Astro documentation](https://docs.astro.build/en/guides/markdown-content/).
...
- **template**: inserts a basic post template

These snippets live in `.vscode/astro-paper.code-snippets`.

:::aside
This is testing **block-level** floating note. It can contain rich
markdown — `code`, [links](https://example.com), and multiple paragraphs.

These snippets live in `.vscode/astro-paper.code-snippets`. If you use VS Code...
:::
...
AstroPaper automatically builds a table of contents from the post headings and displays it in a sticky left rail on desktop-width screens. The rail is hidden on smaller screens to keep the reading column focused.

You do not need to add a `## Table of contents` heading. Use normal `h2` through `h4` headings in the post body and AstroPaper will include them automatically.
